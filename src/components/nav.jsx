    ...
    <nav className={`flex justify-between font-bold text-base bg-[#E4F3FF] items-center md:px-20 px-10 py-6 border-b-[1px] border-b-black border ${menuToggled ? 'fixed' : 'static'} z-50 left-0 right-0`}>
      <div>
        ...
        <ul className={`flex gap-8 pt-[106px] md:pt-0 font-light items-center text-base md:relative md:navv z-50 ${menuToggled ? 'show' : ''} md:flex-row flex-col fixed justify-start md:justify-end h-dvh md:h-auto bg-[#E4F3FF] left-0 right-0 bottom-0 ${menuToggled ? 'translate-x-0' : 'translate-x-[-100%]'} transition-transform duration-500 ease-in-out md:translate-x-0 `}>
          <li onClick={clickNav} className="hover:cursor-pointer"><Link to="/" className="text-[#010536]">Home</Link></li>
          <li onClick={clickNav} className="hover:cursor-pointer"><Link to="/about" className="text-[#010536]">About me</Link></li>
          <li onClick={clickNav} className="hover:cursor-pointer"><Link to="/works" className="text-[#010536]">Works</Link></li>
